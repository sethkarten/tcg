    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                action_masks = th.as_tensor(action_masks, dtype=bool, device=self.device)
                # check which player
                actions, values, log_probs = rollout_policy(obs_tensor, action_masks=action_masks)
                actions = actions.cpu().numpy()
                actions_other, values_other, log_probs_other = rollout_policy_other(obs_tensor, action_masks=action_masks)
                actions_other = actions_other.cpu().numpy()

            # Rescale and perform action

            clipped_actions = np.column_stack((actions, actions_other))
            # print('clipped actions', clipped_actions, actions, actions_other)
            # Clip the actions to avoid out of bound error
            # if isinstance(self.action_space, spaces.Box):
            #     clipped_actions = np.clip(np.hstack([actions, actions_other]), self.action_space.low, self.action_space.high)
            # print('rollout', clipped_actions)
            new_obs, rewards, rewards_other, terminated, infos = env.step(clipped_actions)
            ...
            self._last_episode_starts = terminated
        ...
        if self.update_left and values is not None:
            rollout_buffer.compute_returns_and_advantage(last_values=values, dones=terminated)
        if self.update_right and values_other is not None:
            rollout_buffer_other.compute_returns_and_advantage(last_values=values_other, dones=terminated)
...
class LeaguePPO(IPPO):
    ...
    def train(self, rollout_buffer: RolloutBuffer) -> None:
        ...
        if policy.log_std is not None:
            ...
            self.logger.record(f"train/std{suffix}", None)
