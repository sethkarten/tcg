...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns: List[Callable[[], gymnasium.Env]]):
        ...
        self.buf_rews_other = np.zeros((self.num_envs,), dtype=np.float32)
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        for env_idx in range(self.num_envs):
            obs, (self.buf_rews[env_idx], self.buf_rews_other[env_idx]), terminated, truncated, self.buf_infos[env_idx] = self.envs[env_idx].step(
        ...
        return (self._obs_from_buf(), np.copy(self.buf_rews), np.copy(self.buf_rews_other), np.copy(self.buf_dones), deepcopy(self.buf_infos))
